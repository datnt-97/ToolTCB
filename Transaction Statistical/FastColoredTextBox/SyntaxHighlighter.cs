...
namespace FastColoredTextBoxNS
{
    public class SyntaxHighlighter : IDisposable
    {
        ...
        public readonly Style BrownBoldStyle = new TextStyle(Brushes.Black, Brushes.Violet, FontStyle.Bold | FontStyle.Italic);
        ...
        protected Regex ATMMsgAttributeRegex,
                      ATMMsgClassNameRegex;

        protected Regex ATMMsgCommentRegex1,
                      ATMMsgCommentRegex2,
                      ATMMsgCommentRegex3,
                      ATMMsgCommentRegex4;

        protected Regex ATMMsgKeywordRegex;
        protected Regex ATMMsgNumberRegex;
        protected Regex ATMMsgStringRegex;
        ...
        public virtual void HighlightSyntax(Language language, Range range)
        {
            switch (language)
            {
                ...
                case Language.Lua:
                    ...
                    break;
                case Language.ATMMsg:
                    ATMMsgSyntaxHighlight(range);
                    break;
                default:
                    break;
            }
        ...
        public virtual void AutoIndentNeeded(object sender, AutoIndentEventArgs args)
        {
            ...
            switch (language)
            {
                ...
                case Language.Lua:
                    ...
                    break;
                case Language.ATMMsg:
                    ATMMsgAutoIndentNeeded(sender, args);
                    break;
                default:
                    break;
            }
        ...
        protected void ATMMsgAutoIndentNeeded(object sender, AutoIndentEventArgs args)
        {
            //block {}
            if (Regex.IsMatch(args.LineText, @"^[^""']*\{.*\}[^""']*$"))
                return;
...
";
        ...
        ///////////////////////////
        protected void InitATMMsgRegex()
        {
            //CSharpStringRegex = new Regex( @"""""|@""""|''|@"".*?""|(?<!@)(?<range>"".*?[^\\]"")|'.*?[^\\]'", RegexCompiledOption);
            ...
            ATMMsgCommentRegex1 = new Regex(@"//.*$", RegexOptions.Multiline | RegexCompiledOption);
            ATMMsgCommentRegex2 = new Regex(@"(/\*.*?\*/)|(/\*.*)", RegexOptions.Singleline | RegexCompiledOption);
            ATMMsgCommentRegex3 = new Regex(@"(/\*.*?\*/)|(.*\*/)",
                                            RegexOptions.Singleline | RegexOptions.RightToLeft | RegexCompiledOption);
            ATMMsgCommentRegex4 = new Regex(@"\\1c", RegexOptions.Multiline | RegexCompiledOption);
            ATMMsgNumberRegex = new Regex(@"\b\d+[\.]?\d*([eE]\-?\d+)?[lLdDfF]?\b|\b0x[a-fA-F\d]+\b",
                                          RegexCompiledOption);
            ATMMsgAttributeRegex = new Regex(@"^\s*(?<range>\[.+?\])\s*$", RegexOptions.Multiline | RegexCompiledOption);
            ATMMsgClassNameRegex = new Regex(@"\b(class|struct|enum|interface)\s+(?<range>\w+?)\b", RegexCompiledOption);
            ATMMsgKeywordRegex =
                new Regex(
                    @"\b(abstract|as|base|bool|break|byte|case|catch|char|checked|class|const|continue|decimal|default|delegate|do|double|else|enum|event|explicit|extern|false|finally|fixed|float|for|foreach|goto|if|implicit|in|int|interface|internal|is|lock|long|namespace|new|null|object|operator|out|override|params|private|protected|public|readonly|ref|return|sbyte|sealed|short|sizeof|stackalloc|static|string|struct|switch|this|throw|true|try|typeof|uint|ulong|unchecked|unsafe|ushort|using|virtual|void|volatile|while|add|alias|ascending|descending|dynamic|from|get|global|group|into|join|let|orderby|partial|remove|select|set|value|var|where|yield)\b|#region\b|#endregion\b",
                    RegexCompiledOption);
        ...
        /// <summary>
        /// Highlights ATMMsg code
        /// </summary>
        /// <param name="range"></param>
       
        public virtual void ATMMsgSyntaxHighlight(Range range)
        {
            range.tb.CommentPrefix = "//";
            range.tb.LeftBracket = '(';
            range.tb.RightBracket = ')';
...
";
            ...
            range.ClearStyle(StringStyle, CommentStyle, NumberStyle, AttributeStyle, ClassNameStyle, KeywordStyle);
            //
            if (ATMMsgStringRegex == null)
                InitATMMsgRegex();
            //string highlighting
            range.SetStyle(StringStyle, ATMMsgStringRegex);
            //comment highlighting
            range.SetStyle(CommentStyle, ATMMsgCommentRegex1);
            range.SetStyle(CommentStyle, ATMMsgCommentRegex2);
            range.SetStyle(CommentStyle, ATMMsgCommentRegex3);
            range.SetStyle(FieldStyle, ATMMsgCommentRegex4);
            //number highlighting
            range.SetStyle(NumberStyle, ATMMsgNumberRegex);
            //attribute highlighting
            range.SetStyle(AttributeStyle, ATMMsgAttributeRegex);
            //class name highlighting
            range.SetStyle(ClassNameStyle, ATMMsgClassNameRegex);
            //keyword highlighting
            range.SetStyle(KeywordStyle, ATMMsgKeywordRegex);
