...
namespace Transaction_Statistical
{
    public class CheckedComboBox : Mode_ComboBox
    {
        ...
        internal class Dropdown : Form
        {
            ...
            public Dropdown(CheckedComboBox ccbParent, CustomCheckedListBox cclb)
            {
                this.ccbParent = ccbParent;
                this.cclb = cclb;
                InitializeComponent();
            ...
            public void CloseDropdown(bool enactChanges)
            {
                if (dropdownClosed)
                {
                    ccbParent.FindForm().TopMost = false;
                    return;
                }
              ...
              ccbParent.FindForm().TopMost = true;
               this.Visible = false; 
        ...
        private Dropdown.CustomCheckedListBox List;
        // ******************************** Data ********************************
        /// <summary>
        ...
        {
          //  if (dropdown.IsDisposed) {  this.dropdown = new Dropdown(this, List); this.dropdown.List = List;}
            if (!dropdown.Visible)
            {
                Rectangle rect = RectangleToScreen(this.ClientRectangle);
                dropdown.Location = new Point(rect.X, rect.Y + this.Size.Height+1);
                ...
                dropdown.Show(this);List = dropdown.List;
            }
        }
