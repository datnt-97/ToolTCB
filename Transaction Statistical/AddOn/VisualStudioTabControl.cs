        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            else
            {
                for (var i = 0; i < this.TabCount; i++)
                {
                    ...
                    if (!this.TabPages[i].Text.Equals("Transaction statistical") && !this.TabPages[i].Text.Equals("+"))
                        if (this.ShowClosingMessage)
                        {
                            if (DialogResult.Yes == MessageBox.Show(this.ClosingMessage, "Close [" + this.TabPages[i].Text + "]", MessageBoxButtons.YesNo))
                            {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            for (var i = 0; i <= TabCount - 1; i++)
            {
                ...
                if (i == SelectedIndex)
                {
                    ...
                    if (this.ShowClosingButton && !this.TabPages[i].Text.Equals("Transaction statistical") && !this.TabPages[i].Text.Equals("+"))
                    {
            ...
            // Draws button add
          //  e.Graphics.DrawString("+", new Font("Arial",10), new SolidBrush(this.textColor), Width -20, 2);
